...
| `.cursor/mcp.json` | Browser MCP + Slack MCP |
...
1. **Cursor:** Open project; MCPs (browsermcp, slack) load from `.cursor/mcp.json`.
2. **Slack:** In Cursor Settings → Tools & MCP, complete **Slack OAuth** so the `slack` server connects.
